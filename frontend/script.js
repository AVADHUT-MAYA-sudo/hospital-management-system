...
const manageBranchBtn=document.getElementById("manageBranchBtn");
...
manageBranchBtn.addEventListener("click",()=>{
...
document.getElementById("addStaffForm").addEventListener("submit",
    ...
    const staff={
        ...
        designation:staffDesignation.value,
        specialization:staffSpec.value,
        phone:staffPhone.value,
        qualification:staffQual.value,
        status:"Active"
    };
try{
...
closeSpecificModal("addStaffModal");
loadStaff();
}
catch(err){
console.log(err);
...
const branchRes=await fetch(BRANCH_API);
...
options+=`
<option value="${branch.branch}" ${staff.branch===branch.branch?"selected":""}>
${branch.branch}
</option>`;});
staffTableBody.innerHTML+=`
<tr>
...
<td><button onclick="viewStaff('${staff._id}')">View</button></td>
<td><select onchange="assignBranch('${staff._id}',this.value)">${options}</select></td>
<td>${staff.status}</td>
</tr>`;});
}
catch(err){
...
async function assignBranch(id,branch){
try{
await fetch(`${STAFF_API}/assign/${id}`,{
method:"PUT",
headers:{"Content-Type":"application/json"},
body:JSON.stringify({branch})
});
alert("Branch assigned");
loadStaff();
}
catch(err){
console.log(err);
...
<b>Specialization:</b>${staff.specialization}<br>
<b>Phone:</b>${staff.phone}<br>
<b>Qualification:</b>${staff.qualification}<br>
<b>Branch:</b>${staff.branch || "Not Assigned"}<br>
<b>Status:</b>${staff.status || "Active"}`;
openModal("viewStaffModal");}
/* =========================
ADD BRANCH
========================= */
document.getElementById("addBranchForm")
.addEventListener("submit",
async(e)=>{
e.preventDefault();
const branch={
clinic:branchClinic.value,
branch:branchName.value,
timings:branchTimings.value
};
await fetch(BRANCH_API,{
method:"POST",
headers:{"Content-Type":"application/json"},
body:JSON.stringify(branch)
});
loadBranches();
closeSpecificModal("addBranchModal");}
);
/* =========================
LOAD BRANCHES
========================= */
